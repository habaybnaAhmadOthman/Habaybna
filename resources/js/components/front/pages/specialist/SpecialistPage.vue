  ...
  <div class="library-page">
        ...
        >
          <div class="d-flex flex-wrap-p justify-center-p">
            ...
            >
              <img
                class="object-fit"
                :src="specialistInfo.avatar | defaultAvatar"
                width="240"
                height="240"
                alt=""
              />
            ...
            <div class="pt-30 w-100-p center-p">
              <h1 class="black-2 bold center-p font-20 font-30 mb-10">
                {{ specialistName }}
              </h1>
              <h2 class="black-2 bold font-20 font-24 mb-10">
              ...
              >
                <img
                  class="ml-10 share-img"
                  src="/images/share-color.svg"
                  width="34"
                  height="36"
                  alt=""
                />
          ...
          <div class="mt-50">
            <TabsToggle
              :class="'gap-10 font-14-mo btn-bg-mo'"
              :tabs="tabs"
            ></TabsToggle>
          </div>
    ...
    >
      <div class="d-flex w-100-i">
        <button
          @click="[closeInfoModal($event), openQuestionaireModal($event)]"
          class="btn font-24 w-100"
        >
          متابعة
        </button>
      </div>
...
export default {
  ...
  methods: {
    ...
    async getSpecialistData() {
      ...
      try {
        const data = await this.$store.dispatch(
          `specialist/getSpecialistDetails`,
          this.specialist
        );
        ...
        // appointments
        if (
          data.specialist.appintment &&
          Object.keys(data.specialist.appintment).length > 0
        ) {
          const appt = [];
    ...
    toggletab(tabName) {
      ...
      document
        .querySelector(".tabs-toggle [tab-toggle='" + tabName + "']")
        .classList.add("active");
      document.querySelectorAll("[tab-name]").forEach((tab) => {
        tab.classList.remove("active");
      });
      document
        .querySelectorAll("[tab-name='" + tabName + "']")
        .forEach((tab) => {
    ...
    isFromPaymentPage() {
      if (this.$route.query.payment) {
        if (this.$route.query.payment == "true") {
          ...
          this.setInfoModal(
            "حصل خطأ في عملية الشراء",
            "يرجى التأكد من المعلومات والمحاولة مرة أخرى",
            false,
            false,
            true
          );
  ...
  beforeRouteEnter(to, from, next) {
    if (from.params.course) localStorage.setItem("active_tab", "courses");
    next((vm) => {
      vm.prevRoute = from;
    });
  },
