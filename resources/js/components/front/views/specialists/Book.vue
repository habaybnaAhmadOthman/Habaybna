<template>
  <div class="book-box" v-if="intervals.length > 0">
    ...
    <div class="price-box w-50 mt-30 d-flex align-center space-between w-100-p">
      <p class="font-20 main-color bold d-flex align-center">
        <img
          class="ml-10"
          ...
          height="30"
          alt="الأخصائيين"
        /><span class="top-2"> USD {{ callPrice }}</span>
      </p>
      <p class="font-20 main-color bold d-flex align-center">
        <img
          class="ml-10"
  ...
  <div v-else>لا يوجد مواعيد متاحة في الوقت الحالي</div>
</template>
...
export default {
  ...
  methods: {
      ...
      for (const i in this.daysWithIntervals) {
        ...
        if (dayObj.id == id) {
          ...
          if (
            ...
            this.daysWithIntervals[this.selectedDay].intervals[
              this.selectedTime
            ].selected = false;
            this.selectedTime = null;
          }
    ...
    saveSelectedApt(isWithCobone) {
      const selectedDayIndex = Object.keys(this.daysWithIntervals).indexOf(
        this.selectedDay
      );
      const obj = {
    ...
    getAvailableTimes() {
      ...
      if (this.intervals.length) this.$emit("show-appointment-tab", true);
      else this.$emit("show-appointment-tab", false);
    ...
    getSavedAppointemnt() {
      ...
      if (Object.keys(booked).length > 0 && params["is-logged-in"]) {
        ...
        if (!booked.isWithCobone) this.bookAppointment();
      }
    },
  },
  computed: {
    ...
    selectedAppointment() {
      return this.daysWithIntervals[this.selectedDay].intervals[
        this.selectedTime
      ];
    },
    isLoggedIn() {
    ...
    canMakeCalls() {
        ...
        if(this.isLoading && this.$store.getters["user/userData"].canMakeCalls){
